...
const STATUS_COLORS = {
  draft: '#64748b', sent: '#60a5fa', paid: '#22c55e',
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(f); }}>
      ...
      <div style={{ marginBottom: 12, padding: 12, background: '#0f172a', borderRadius: 8 }}>
        <div style={{ fontSize: 10, color: '#94a3b8', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Customer</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          <div>
            <label style={{ fontSize: 10, color: '#64748b' }}>From CRM Company (optional)</label>
          ...
          <div>
            <label style={{ fontSize: 10, color: '#64748b' }}>From CRM Contact (optional)</label>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 10, color: '#94a3b8', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Line items</div>
        {f.line_items.map((it, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr 30px', gap: 6, marginBottom: 6 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', fontSize: 11, color: '#94a3b8', padding: '0 6px' }}>
              {money(Number(it.quantity) * Number(it.unit_price), f.currency)}
            </div>
            <button type="button" onClick={() => removeLine(i)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer' }}>
              <Trash2 size={12} />
            </button>
          </div>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 200px', gap: 8, alignItems: 'end', marginBottom: 12 }}>
        <div>
          <label style={{ fontSize: 10, color: '#64748b' }}>Notes (payment terms, thank-you, etc.)</label>
          <textarea className="field-input" rows={3} value={f.notes} onChange={(e) => set('notes', e.target.value)} maxLength={2000} />
        </div>
        <div style={{ padding: 12, background: '#0f172a', borderRadius: 8, fontSize: 11 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', color: '#94a3b8', marginBottom: 4 }}>
            <span>Tax</span>
            <input type="number" step="0.01" min={0} max={100} value={f.tax_pct} onChange={(e) => set('tax_pct', parseFloat(e.target.value) || 0)} style={{ width: 60, fontSize: 11, padding: '2px 4px', background: '#0c1222', border: '1px solid #1e293b', borderRadius: 4, color: '#e2e8f0', textAlign: 'right' }} />
            <span>% = {money(taxAmount, f.currency)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, color: '#e2e8f0', fontSize: 13, paddingTop: 6, borderTop: '1px solid #1e293b', marginTop: 6 }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {summary && (
        <div style={{ padding: '0 24px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 8 }}>
          {[
            { label: 'Outstanding', value: money(summary.outstanding.total), sub: `${summary.outstanding.count} invoices`, icon: Clock, color: '#60a5fa' },
            { label: 'Overdue', value: money(summary.overdue.total), sub: `${summary.overdue.count} invoices`, icon: AlertTriangle, color: '#ef4444' },
            { label: 'Paid', value: money(summary.paid.total), sub: `${summary.paid.count} invoices`, icon: CheckCircle, color: '#22c55e' },
            { label: 'Draft', value: money(summary.draft.total), sub: `${summary.draft.count} invoices`, icon: FileText, color: '#94a3b8' },
                ...
                {invoices.map((inv) => (
                  <tr key={inv.id}>
                    ...
                    <td>
                      ...
                      {inv.customer_email && <div style={{ fontSize: 10, color: '#64748b' }}>{inv.customer_email}</div>}
                    </td>
                    ...
                    <td style={{ display: 'flex', gap: 4 }}>
                      ...
                      {inv.status === 'draft' && <button className="btn-ghost" style={{ padding: 4, color: '#60a5fa' }} onClick={() => handleMarkStatus(inv, 'sent')} title="Mark sent"><Send size={11} /></button>}
                      {inv.status === 'sent' && <button className="btn-ghost" style={{ padding: 4, color: '#22c55e' }} onClick={() => handleMarkStatus(inv, 'paid')} title="Mark paid"><CheckCircle size={11} /></button>}
                      <button className="btn-ghost" style={{ padding: 4 }} onClick={() => setModal({ record: inv })}><Edit3 size={11} /></button>
                      <button className="btn-ghost" style={{ padding: 4, color: '#f87171' }} onClick={() => handleDelete(inv)}><Trash2 size={11} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
