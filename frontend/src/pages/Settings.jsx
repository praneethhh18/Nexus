...
import { downloadFullExport, downloadFullBackup, getBackupInfo, listIntakeKeys, createIntakeKey, revokeIntakeKey } from '../services/tags';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="page-body">
        ...
        {isAdmin && <IntakeKeysPanel flash={flash} />}
...
// ── Public-form intake keys (admin-only) ─────────────────────────────────────
// Each key gives a public form (or any third-party tool) a way to drop
// leads straight into this workspace's CRM via POST /api/public/leads.
// Keys are created server-side, hashed at rest, and shown raw exactly
// once. Revoking a key kills its inbound flow without affecting any
// already-captured contacts.
function IntakeKeysPanel({ flash }) {
  ...
  const sampleSnippet = activeKey ? buildSampleSnippet(activeKey.key || activeKey.key_prefix) : '';
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--color-text-muted)', lineHeight: 1.55, marginBottom: 10 }}>
            Generate a key, paste a 5-line snippet onto your website, and inbound
            leads land directly in this workspace's CRM tagged
            {' '}<code>source: public_form</code>. Keys are revocable, rate-limited,
            and never echoed back after creation — copy yours when shown.
          </div>

          {/* Just-created banner — show the raw key once */}
          {justCreated && (
            <div style={{
...
function buildSampleSnippet(keyOrPlaceholder) {
  // If we have the raw key, use it. Otherwise show the prefix as a hint
  // and tell the user to substitute their own.
